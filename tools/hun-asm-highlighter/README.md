...
> Apple의 ARM64 ABI는 가변인자(variadic) 함수를 부를 때 고정 인자
> 이후는 반드시 스택에 실어야 하는데, 이걸 빠뜨리면 값이 조용히
> 날아가 버립니다 (레지스터에만 넣으면 컴파일은 되지만 런타임에
> 쓰레기 값이 나옵니다).
...
- variadic 함수(`printf`/`scanf` 등) 호출 시 스택 인자 세팅 누락에 대한 힌트 제공
